...
(function($) {
    ...
    $.verto.dialog.prototype.stopRinging = function() {
        ...
        if (dialog.verto.ringer) {
            ...
            if(dialog.verto.aswatRinger) dialog.verto.aswatRinger.pause();
        }
    };
    ...
    $.verto.dialog.prototype.indicateRing = function() {
        var dialog = this;
        if (dialog.verto.ringer) {
            dialog.verto.aswatRinger = new Audio(dialog.verto.options.ringFile);
            dialog.verto.ringer.attr("src", dialog.verto.options.ringFile)[0].play();
            if(dialog.verto.aswatRinger) dialog.verto.aswatRinger.play();
            setTimeout(function() {
                dialog.stopRinging();
                if (dialog.state == $.verto.enum.state.ringing) {
                    dialog.indicateRing();
